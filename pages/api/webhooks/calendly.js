...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Отправка в Meta (v18.0 + дополнительные параметры)
    const fbResponse = await fetch(`https://graph.facebook.com/v18.0/${FACEBOOK_DATASET_ID}/events`, {
      ...
      body: JSON.stringify({
        data: [{
          ...
          event_source_url: 'https://kove.services', // Добавили URL
          user_data: {
            em: [hashedEmail]
          }
        }],
    ...
    // ВЫВОДИМ ОТВЕТ МЕТЫ В ЛОГИ (это покажет, сколько событий Meta реально получила)
